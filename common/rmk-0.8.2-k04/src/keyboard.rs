...
use rmk_types::keycode::{from_ascii, KeyCode};
...
const K04_POINTING_MAGIC: [u8; 4] = *b"K04P";
const K04_CUSTOM_MODE: u8 = 4;
const K04_USER_LNEXT: u8 = 5;
const K04_USER_LPREV: u8 = 6;
const K04_USER_LG_TOGGLE: u8 = 7;
const K04_USER_LG_SYNC: u8 = 8;
const K04_USER_LG_SET_EN: u8 = 9;
const K04_USER_LG_SET_RU: u8 = 10;
const K04_USER_LG_SET_M0: u8 = 11;
const K04_USER_LG_SET_M1M2: u8 = 12;
const K04_USER_LG_SET_DFLT: u8 = 13;
const K04_USER_LG_WORD: u8 = 14;
const K04_USER_LG_MOD: u8 = 15;
const K04_USER_LG_STORE: u8 = 16;
const K04_USER_LG_REVERT: u8 = 17;
const K04_USER_LG_TG_MAC: u8 = 18;
...
const K04_USER_SNIPER_MODE: u8 = 28;
const K04_USER_SCROLL_MODE: u8 = 29;
const K04_USER_TEXT_MODE: u8 = 30;
const K04_USER_LEFT_SNIPER_MODE: u8 = 31;
const K04_USER_LEFT_SCROLL_MODE: u8 = 32;
const K04_USER_LEFT_TEXT_MODE: u8 = 33;
const K04_USER_RIGHT_SNIPER_MODE: u8 = 34;
const K04_USER_RIGHT_SCROLL_MODE: u8 = 35;
const K04_USER_RIGHT_TEXT_MODE: u8 = 36;
...
const K04_LANG_RU: u8 = 1;
const K04_RUEN_TG_DEFAULT: u8 = 0;
const K04_RUEN_TG_M0: u8 = 1;
const K04_RUEN_TG_M1M2: u8 = 2;
static K04_MODE_PRESS_MS: core::sync::atomic::AtomicU32 = core::sync::atomic::AtomicU32::new(0);
pub static K04_MOUSE_BUTTONS: core::sync::atomic::AtomicU8 = core::sync::atomic::AtomicU8::new(0);
...
pub struct Keyboard<'a, const ROW: usize, const COL: usize, const NUM_LAYER: usize, const NUM_ENCODER: usize = 0> {
    ...
    k04_ruen_lang: u8,
    k04_ruen_stored_lang: u8,
    k04_ruen_toggle_mode: u8,
    k04_ruen_mac_layout: bool,
    k04_ruen_english_word: bool,
    k04_ruen_mod_press_ms: u32,
    ...
    Keyboard<'a, ROW, COL, NUM_LAYER, NUM_ENCODER>
{
    pub fn new(keymap: &'a RefCell<KeyMap<'a, ROW, COL, NUM_LAYER, NUM_ENCODER>>) -> Self {
        Keyboard {
            ...
            combo_on: true,
            k04_ruen_lang: K04_LANG_EN,
            k04_ruen_stored_lang: K04_LANG_EN,
            k04_ruen_toggle_mode: K04_RUEN_TG_DEFAULT,
            k04_ruen_mac_layout: false,
            k04_ruen_english_word: false,
            k04_ruen_mod_press_ms: 0,
            #[cfg(feature = "controller")]
    ...
    async fn process_action_key(&mut self, key: KeyCode, event: KeyboardEvent) {
        ...
        if event.pressed {
            ...
            self.k04_check_english_word_end(key, event).await;
        }
        ...
        if key.is_consumer() {
            ...
            self.process_k04_kb_user_key(key, event).await;
        } else if key.is_user() {
    ...
    async fn process_action_mouse(&mut self, key: KeyCode, event: KeyboardEvent) {
        if key.is_mouse_key() {
            ...
            if matches!(
                ...
                K04_MOUSE_BUTTONS.store(
                    self.mouse_report.buttons,
    ...
    async fn process_user(&mut self, key: KeyCode, event: KeyboardEvent) {
        ...
        let id = (key as u16 - KeyCode::User0 as u16) as u8;
        if self.process_k04_ergohaven_user_key(id, event).await {
            return;
        }
    }

    async fn process_k04_kb_user_key(&mut self, key: KeyCode, event: KeyboardEvent) {
        let id = (key as u16 - KeyCode::Kb0 as u16) as u8;
        self.process_k04_ble_user_id(id, event).await;
    }

    async fn process_k04_ble_user_id(&mut self, id: u8, event: KeyboardEvent) {
        #[cfg(feature = "_ble")]
        {
    ...
    async fn process_k04_ergohaven_user_key(&mut self, id: u8, event: KeyboardEvent) -> bool {
        if self.process_k04_vial_ble_user_key(id, event).await {
            return true;
        }

        if self.process_k04_pointing_mode_user_key(id, event.pressed).await {
            return true;
        }

        match id {
            0 if event.pressed => self.k04_send_ascii("Product: K:04\nFW: RMK K04\n", event).await,
            1 => self.k04_hold_with_mod(KeyCode::Right, ModifierCombination::LCTRL, event).await,
            2 => self.k04_hold_with_mod(KeyCode::Left, ModifierCombination::LCTRL, event).await,
            3 => self.k04_hold_with_mod(KeyCode::Tab, ModifierCombination::LALT, event).await,
            4 => self.k04_hold_with_mod(KeyCode::Tab, ModifierCombination::LALT | ModifierCombination::LSHIFT, event).await,
            K04_USER_LNEXT if event.pressed => self.k04_move_layer(true),
            K04_USER_LPREV if event.pressed => self.k04_move_layer(false),
            K04_USER_BATTERY_LEVEL if event.pressed => {
                #[cfg(feature = "_nrf_ble")]
                crate::input_device::adc::request_battery_adc_sample();
                send_controller_event(&mut self.controller_pub, ControllerEvent::BatteryLevelRequest);
                #[cfg(feature = "split")]
                send_controller_event(
                    &mut self.controller_pub,
                    ControllerEvent::SplitBatteryLevelRequest(K04_RIGHT_SPLIT_PERIPHERAL_ID),
                );
            }
            K04_USER_LG_MOD => {
                if event.pressed {
                    self.k04_ruen_mod_press_ms = Instant::now().as_millis() as u32;
                    self.k04_toggle_lang(event).await;
                } else {
                    let elapsed = (Instant::now().as_millis() as u32).wrapping_sub(self.k04_ruen_mod_press_ms);
                    if elapsed >= K04_MODE_TAPPING_TERM_MS {
                        self.k04_toggle_lang(event).await;
                    }
                }
            }
            _ if !event.pressed => {}
            K04_USER_LG_TOGGLE => self.k04_toggle_lang(event).await,
            K04_USER_LG_SYNC => self.k04_ruen_lang ^= 1,
            K04_USER_LG_SET_EN => self.k04_set_lang(K04_LANG_EN, event).await,
            K04_USER_LG_SET_RU => self.k04_set_lang(K04_LANG_RU, event).await,
            K04_USER_LG_SET_M0 => self.k04_ruen_toggle_mode = K04_RUEN_TG_M0,
            K04_USER_LG_SET_M1M2 => self.k04_ruen_toggle_mode = K04_RUEN_TG_M1M2,
            K04_USER_LG_SET_DFLT => self.k04_ruen_toggle_mode = K04_RUEN_TG_DEFAULT,
            K04_USER_LG_WORD => {
                if self.k04_ruen_lang == K04_LANG_RU && !self.k04_ruen_english_word {
                    self.k04_ruen_english_word = true;
                    self.k04_set_lang(K04_LANG_EN, event).await;
                }
            }
            K04_USER_LG_STORE => {
                self.k04_ruen_stored_lang = self.k04_ruen_lang;
            }
            K04_USER_LG_REVERT => self.k04_set_lang(self.k04_ruen_stored_lang, event).await,
            K04_USER_LG_TG_MAC => self.k04_ruen_mac_layout = !self.k04_ruen_mac_layout,
            _ => return false,
        }
        true
    }

    async fn process_k04_vial_ble_user_key(&mut self, id: u8, event: KeyboardEvent) -> bool {
        #[cfg(feature = "_ble")]
        {
            use crate::ble::profile::BleProfileAction;
            use crate::channel::BLE_PROFILE_CHANNEL;
            ...
            match id {
                K04_USER_BT_OUTPUT => {
                    if !event.pressed {
                        BLE_PROFILE_CHANNEL.send(BleProfileAction::SetConnection(1)).await;
                    }
                    return true;
                }
                K04_USER_USB_OUTPUT => {
                    if !event.pressed {
                        BLE_PROFILE_CHANNEL.send(BleProfileAction::SetConnection(0)).await;
                    }
                    return true;
                }
                _ => {}
            ...
            let ble_id = match id {
                K04_USER_BT_PROFILE0 => 0,
                K04_USER_BT_PROFILE1 => 1,
                K04_USER_BT_PROFILE2 => 2,
                K04_USER_BT_PROFILE3 => 3,
                K04_USER_BT_PROFILE4 => 4,
                K04_USER_BT_NEXT => NUM_BLE_PROFILE as u8,
                K04_USER_BT_PREV => NUM_BLE_PROFILE as u8 + 1,
                K04_USER_BT_CLEAR => NUM_BLE_PROFILE as u8 + 2,
                K04_USER_BT_TOGGLE => NUM_BLE_PROFILE as u8 + 3,
                K04_USER_BT_CLEAR_PEER => NUM_BLE_PROFILE as u8 + 4,
                _ => return false,
            };
            self.process_k04_ble_user_id(ble_id, event).await;
            return true;
        }
    ...
    async fn k04_set_lang(&mut self, lang: u8, event: KeyboardEvent) {
        if self.k04_ruen_lang == lang {
            return;
        }
        match self.k04_ruen_toggle_mode {
            K04_RUEN_TG_M0 => self.k04_execute_macro_now(0, event).await,
            K04_RUEN_TG_M1M2 => self.k04_execute_macro_now(if lang == K04_LANG_EN { 1 } else { 2 }, event).await,
            _ => {
                self.k04_tap_with_mods(KeyCode::Space, ModifierCombination::LGUI, event).await;
                Timer::after_millis(50).await;
            }
        }
        self.k04_ruen_lang = lang;
    }

    async fn k04_toggle_lang(&mut self, event: KeyboardEvent) {
        let lang = if self.k04_ruen_lang == K04_LANG_EN {
            K04_LANG_RU
        } else {
            K04_LANG_EN
        };
        self.k04_set_lang(lang, event).await;
    }

    async fn k04_execute_macro_now(&mut self, macro_idx: u8, event: KeyboardEvent) {
        let macro_event = KeyboardEvent {
            pressed: false,
            pos: event.pos,
        };
        self.execute_macro(macro_idx, macro_event).await;
    }

    fn k04_move_layer(&mut self, next: bool) {
        let current = self.keymap.borrow().get_activated_layer();
        let layer = if next {
            if current + 1 >= NUM_LAYER as u8 { 0 } else { current + 1 }
        } else if current == 0 {
            NUM_LAYER as u8 - 1
        } else {
            current - 1
        ...
        for i in 0..NUM_LAYER as u8 {
            if i != self.keymap.borrow().get_default_layer() {
                self.keymap.borrow_mut().deactivate_layer(i);
            }
        }
    ...
    async fn k04_check_english_word_end(&mut self, key: KeyCode, event: KeyboardEvent) {
        if !self.k04_ruen_english_word {
            return;
        }
        match key {
            KeyCode::Space | KeyCode::Enter | KeyCode::Escape | KeyCode::Minus => {
                self.k04_ruen_english_word = false;
                self.caps_word.deactivate();
                self.k04_set_lang(K04_LANG_RU, event).await;
            }
            _ => {}
        }
    }

    async fn k04_send_ascii(&mut self, text: &str, event: KeyboardEvent) {
        for byte in text.bytes() {
            let (key, shifted) = from_ascii(byte);
            if shifted {
                self.k04_tap_shifted(key, event).await;
            } else {
                self.k04_tap_key(key, event).await;
            }
            Timer::after_millis(2).await;
        }
    }

    async fn k04_hold_with_mod(&mut self, key: KeyCode, mods: ModifierCombination, event: KeyboardEvent) {
        if event.pressed {
            self.register_modifiers(mods);
            self.register_keycode(key, event);
            self.send_keyboard_report_with_resolved_modifiers(true).await;
        } else {
            self.unregister_keycode(key, event);
            self.unregister_modifiers(mods);
            self.send_keyboard_report_with_resolved_modifiers(false).await;
        }
    }

    async fn k04_tap_key(&mut self, key: KeyCode, event: KeyboardEvent) {
        self.register_keycode(key, event);
        self.send_keyboard_report_with_resolved_modifiers(true).await;
    ...
    async fn k04_tap_shifted(&mut self, key: KeyCode, event: KeyboardEvent) {
        self.k04_tap_with_mods(key, ModifierCombination::LSHIFT, event).await;
    }

    async fn k04_tap_with_mods(&mut self, key: KeyCode, mods: ModifierCombination, event: KeyboardEvent) {
        self.register_modifiers(mods);
        self.register_keycode(key, event);
        self.send_keyboard_report_with_resolved_modifiers(true).await;
        Timer::after_millis(12).await;
    ...
    async fn k04_tap_ruen_punct(
        ...
        if self.k04_ruen_lang == K04_LANG_EN {
            self.k04_tap_key(en_key, event).await;
        } else if self.k04_ruen_mac_layout {
            if ru_shifted {
                self.k04_tap_shifted(ru_mac_key, event).await;
            } else {
                self.k04_tap_key(ru_mac_key, event).await;
            }
        } else if ru_shifted {
            self.k04_tap_shifted(ru_pc_key, event).await;
        } else {
            self.k04_tap_key(ru_pc_key, event).await;
        }
    }

    async fn k04_tap_en_symbol(&mut self, key: KeyCode, event: KeyboardEvent) {
        let restore = self.k04_ruen_lang == K04_LANG_RU;
        if restore {
            self.k04_set_lang(K04_LANG_EN, event).await;
        }
        self.k04_tap_key(key, event).await;
    ...
    async fn k04_tap_en_symbol_shifted(&mut self, key: KeyCode, event: KeyboardEvent) {
        let restore = self.k04_ruen_lang == K04_LANG_RU;
        if restore {
            self.k04_set_lang(K04_LANG_EN, event).await;
        }
    ...
    async fn k04_tap_ru_letter(&mut self, key: KeyCode, event: KeyboardEvent) {
        if self.k04_ruen_lang == K04_LANG_RU {
            self.k04_tap_key(key, event).await;
        }
    }

    async fn process_k04_pointing_mode_user_key(&mut self, id: u8, pressed: bool) -> bool {
        let (mode, side) = match id {
            K04_USER_SNIPER_MODE => (1, 0xFF),
            K04_USER_SCROLL_MODE => (2, 0xFF),
            K04_USER_TEXT_MODE => (3, 0xFF),
            K04_USER_LEFT_SNIPER_MODE => (1, 0),
            K04_USER_LEFT_SCROLL_MODE => (2, 0),
            K04_USER_LEFT_TEXT_MODE => (3, 0),
            K04_USER_RIGHT_SNIPER_MODE => (1, 1),
            K04_USER_RIGHT_SCROLL_MODE => (2, 1),
            K04_USER_RIGHT_TEXT_MODE => (3, 1),
            _ => return false,
        };
        let now_ms = Instant::now().as_millis() as u32;
        let mut data = [0u8; 16];
        data[0..4].copy_from_slice(&K04_POINTING_MAGIC);
        data[4] = K04_CUSTOM_MODE;
        data[5] = mode;
        data[6] = pressed as u8;
        data[8] = side;
        if pressed {
            K04_MODE_PRESS_MS.store(now_ms, core::sync::atomic::Ordering::Relaxed);
        } else {
            let elapsed = now_ms.wrapping_sub(K04_MODE_PRESS_MS.load(core::sync::atomic::Ordering::Relaxed));
            data[7] = (elapsed < K04_MODE_TAPPING_TERM_MS) as u8;
        }
        if EVENT_CHANNEL.is_full() {
            let _ = EVENT_CHANNEL.receive().await;
        }
        EVENT_CHANNEL.send(Event::Custom(data)).await;
        true
    }
    ...
    pub(crate) async fn send_mouse_report(&mut self) {
        ...
        K04_MOUSE_BUTTONS.store(self.mouse_report.buttons, core::sync::atomic::Ordering::Relaxed);
