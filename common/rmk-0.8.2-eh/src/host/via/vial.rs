...
use rmk_types::protocol::vial::{
    SettingKey, VIAL_COMBO_MAX_LENGTH, VIAL_EP_SIZE, VIAL_PROTOCOL_VERSION, VialCommand,
    VialDynamic,
};
...
const STANDARD_SETTING_KEYS: &[u16] = &[0x02, 0x06, 0x07, 0x12, 0x13, 0x16, 0x17, 0x1A, 0x1B];

fn write_setting_query(output: &mut [u8], after: u16) {
    output.fill(0xFF);

    let mut offset = 0;
    for key in STANDARD_SETTING_KEYS
        .iter()
        .copied()
        .filter(|key| *key > after)
        .chain(
            crate::vial_settings::SETTING_KEYS
                .iter()
                .copied()
                .filter(|key| *key > after),
        )
    {
        ...
        LittleEndian::write_u16(&mut output[offset..offset + 2], key);
        offset += 2;
    }
}
...
>(
    ...
    #[cfg(feature = "vial_lock")] locker: &mut super::vial_lock::VialLock<
        '_,
        ROW,
        COL,
        NUM_LAYER,
    ...
    match vial_command {
        ...
        VialCommand::BehaviorSettingQuery => {
            ...
            write_setting_query(&mut report.input_data, value);
        }
        VialCommand::GetBehaviorSetting => {
            report.input_data.fill(0xFF);
            let value = u16::from_le_bytes([report.output_data[2], report.output_data[3]]);
            report.input_data[0] = 0;
            if crate::vial_settings::get_setting(value, &mut report.input_data[1..]).is_some() {
                return;
            }
            match value.into() {
                ...
                SettingKey::OneShotTimeout => {
                    let one_shot_timeout =
                        keymap.borrow().behavior.one_shot.timeout.as_millis() as u16;
                ...
                SettingKey::PriorIdleTime => {
                    let prior_idle_time =
                        keymap.borrow().behavior.morse.prior_idle_time.as_millis() as u16;
        ...
        VialCommand::SetBehaviorSetting => {
            ...
            if let Some(settings) = crate::vial_settings::set_setting(key, &report.output_data[4..])
            {
                #[cfg(feature = "storage")]
                FLASH_CHANNEL
                    .send(FlashOperationMessage::VialSettings(settings))
                    .await;
                return;
            }
            match key.into() {
                ...
                SettingKey::ComboTimeout => {
                    ...
                    keymap.borrow_mut().behavior.combo.timeout =
                        Duration::from_millis(combo_timeout as u64);
                    #[cfg(feature = "storage")]
                    FLASH_CHANNEL
                ...
                SettingKey::OneShotTimeout => {
                    let timeout_time =
                        u16::from_le_bytes([report.output_data[4], report.output_data[5]]);
                    keymap.borrow_mut().behavior.one_shot.timeout =
                        Duration::from_millis(timeout_time as u64);
                    #[cfg(feature = "storage")]
                    FLASH_CHANNEL
                ...
                SettingKey::TapCapslockInterval => {
                    ...
                    #[cfg(feature = "storage")]
                    FLASH_CHANNEL
                        .send(FlashOperationMessage::TapCapslockInterval(
                ...
                SettingKey::PriorIdleTime => {
                    let prior_idle_time =
                        u16::from_le_bytes([report.output_data[4], report.output_data[5]]);
                    keymap.borrow_mut().behavior.morse.prior_idle_time =
        ...
        VialCommand::QmkSettingsReset => {
            let settings = crate::vial_settings::reset();
            #[cfg(feature = "storage")]
            FLASH_CHANNEL
                .send(FlashOperationMessage::VialSettings(settings))
                .await;
            ...
            match vial_dynamic {
                ...
                VialDynamic::DynamicVialMorseSet => {
                    ...
                    if morse_idx < morses_len {
                        ...
                        if let Some(morse) =
                            keymap.borrow_mut().behavior.morse.morses.get_mut(morse_idx)
                        {
                            ...
                            let double_tap = from_via_keycode(LittleEndian::read_u16(
                                &report.output_data[8..10],
                            ));
                            let hold_after_tap = from_via_keycode(LittleEndian::read_u16(
                                &report.output_data[10..12],
                            ));
                    ...
                    if let Some(Some(combo)) = combos.get(combo_idx) {
                        ...
                        for i in 0..VIAL_COMBO_MAX_LENGTH {
                            LittleEndian::write_u16(
                                ...
                                to_via_keycode(
                                    *combo.config.actions.get(i).unwrap_or(&KeyAction::No),
                                ),
                            );
                        }
                        // Combo output
                ...
                VialDynamic::DynamicVialComboSet => {
                    ...
                    let (actions, output) = {
                        ...
                        combos[combo_idx] = if !actions.iter().any(|&x| x != KeyAction::No)
                            && output == KeyAction::No
                        {
        ...
        VialCommand::SetEncoder => {
            ...
            if let Some(encoder) = _encoder {
                ...
                FLASH_CHANNEL
                    .send(FlashOperationMessage::VialMessage(KeymapData::Encoder(
                        EncoderKeymap {
                            idx: index,
                            layer,
                            action: encoder,
    ...
    fn test_combo_serialization_deserialization() {
        ...
        match deserialized_data {
            (StorageData::VialData(KeymapData::Combo(idx, deserialized_config)), _) => {
                ...
                // actions
                assert_eq!(
                    deserialized_config.actions.len(),
                    combo_config.actions.len()
                );
                for (original, deserialized) in combo_config
                    .actions
                    .iter()
                    .zip(deserialized_config.actions.iter())
                {
