...
pub(crate) fn from_via_keycode(via_keycode: u16) -> KeyAction {
    match via_keycode {
        ...
        0x2000..=0x3FFF => {
            ...
            let keycode = (via_keycode & 0x00FF).into();
            let modifier =
                ModifierCombination::from_packed_bits(((via_keycode >> 8) & 0b11111) as u8);
            KeyAction::TapHold(
                Action::Key(keycode),
        ...
        0x7700..=0x77FF => {
            // Macro
            KeyAction::Single(Action::TriggerMacro((via_keycode & 0xFF) as u8))
        }
        0x7800..=0x783F => {
...
mod test {
    ...
    fn test_convert_via_keycode_to_key_action() {
        ...
        assert_eq!(
            KeyAction::TapHold(
                Action::Key(KeyCode::A),
                Action::Modifier(ModifierCombination::new_from(
