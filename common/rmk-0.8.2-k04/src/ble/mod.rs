...
pub enum BleState {
    ...
    Advertising,
    // The BLE is trying to reconnect to the active bonded host
    Reconnecting,
    // The BLE is advertising without a known bonded host
    Pairing,
...
async fn advertise_until_connected<'a, 'b, C: Controller>(
    ...
    if let Some(addr) = active_peer {
        ...
        for _ in 0..DIRECTED_RECONNECT_ROUNDS {
            for peer in [
                Address {
