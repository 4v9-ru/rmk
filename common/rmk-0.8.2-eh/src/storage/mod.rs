...
#[cfg(feature = "vial")]
use crate::vial_settings::{self, VialSettingsData};
...
impl StorageKeys {
    pub(crate) fn from_u8(value: u8) -> Option<Self> {
        match value {
            ...
            #[cfg(feature = "vial")]
            10 => Some(StorageKeys::VialSettings),
            #[cfg(all(feature = "_ble", feature = "split"))]
...
pub(crate) fn get_keymap_key<const ROW: usize, const COL: usize, const NUM_LAYER: usize>(
    ...
    0x1000
        + (keymap_key.layer as usize * COL * ROW
            + keymap_key.row as usize * COL
            + keymap_key.col as usize) as u32
}
...
impl StorageData {
    ...
    const fn key(&self) -> u32 {
        match self {
            ...
            #[cfg(feature = "vial")]
            Self::VialSettings(_) => StorageKeys::VialSettings as u32,
        }
    }
}
...
impl Value<'_> for StorageData {
    fn serialize_into(&self, buffer: &mut [u8]) -> Result<usize, SerializationError> {
        ...
        match self {
            ...
            #[cfg(feature = "_ble")]
            Self::ActiveBleProfile(d) => {
                ser_storage_variant!(buffer, StorageKeys::ActiveBleProfile, d)
            }
            #[cfg(feature = "host")]
            Self::VialData(vial_data) => vial_data.serialize_into(buffer),
            #[cfg(feature = "vial")]
            Self::VialSettings(d) => ser_storage_variant!(buffer, StorageKeys::VialSettings, d),
        }
    }
        ...
        Self: Sized,
    {
        ...
        match key {
            StorageKeys::StorageConfig => {
                let (data, unused) = postcard::take_from_bytes(&buffer[1..])
            ...
            #[cfg(feature = "vial")]
            StorageKeys::VialSettings => {
                let (data, unused) = postcard::take_from_bytes(&buffer[1..])
                    .map_err(postcard_error_to_serialization_error)?;
                let size = buffer.len() - unused.len();
                Ok((Self::VialSettings(data), size))
            }
            #[cfg(feature = "host")]
            ...
            | StorageKeys::MorseData => {
                ...
                KeymapData::deserialize_from(buffer)
                    .map(|(data, size)| (Self::VialData(data), size))
            }
        }
    }
}
...
> Storage<F, ROW, COL, NUM_LAYER, NUM_ENCODER>
{
    pub async fn new(
        ...
        #[cfg(feature = "vial")]
        storage.read_vial_settings().await.ok();
            ...
            match match info {
                ...
                FlashOperationMessage::VialMessage(vial_data) => match vial_data {
                    ...
                    KeymapData::Encoder(encoder_config) => {
                        ...
                        let key = get_encoder_config_key::<NUM_ENCODER>(
                            encoder_config.idx,
                            encoder_config.layer,
                        );
                        store_item(
                            &mut self.flash,
                            self.storage_range.clone(),
                ...
                #[cfg(feature = "vial")]
                FlashOperationMessage::VialSettings(settings) => {
                    let settings = vial_settings::apply(settings);
                    store_item::<u32, StorageData, _>(
                        &mut self.flash,
                        self.storage_range.clone(),
                        &mut storage_cache,
                        &mut self.buffer,
                        &(StorageKeys::VialSettings as u32),
                        &StorageData::VialSettings(settings),
                    )
                    .await
                }
    ...
    #[cfg(feature = "vial")]
    pub(crate) async fn read_vial_settings(&mut self) -> Result<(), ()> {
        if let Some(StorageData::VialSettings(settings)) = fetch_item::<u32, StorageData, _>(
            &mut self.flash,
            self.storage_range.clone(),
            &mut NoCache::new(),
            &mut self.buffer,
    ...
    async fn initialize_storage_with_config(
        ...
        #[cfg(feature = "vial")]
        {
            let vial_settings = StorageData::VialSettings(vial_settings::reset());
            store_item(
                &mut self.flash,
                self.storage_range.clone(),
                &mut cache,
