...
use rmk_types::keycode::{KeyCode, from_ascii};
...
const EH_COMMON_USER_PRINT_INFO: u8 = 17;
const EH_COMMON_USER_WORD_NEXT: u8 = 18;
const EH_COMMON_USER_WORD_PREV: u8 = 19;
const EH_COMMON_USER_WINDOW_NEXT: u8 = 20;
const EH_COMMON_USER_WINDOW_PREV: u8 = 21;
const EH_COMMON_USER_LAYER_NEXT: u8 = 22;
const EH_COMMON_USER_LAYER_PREV: u8 = 23;
...
impl CapsWordState {
    ...
    fn check(&mut self, key: KeyCode) {
        if let CapsWordState::Activated {
            timer,
            shift_current,
        } = self
        {
    ...
    Keyboard<'a, ROW, COL, NUM_LAYER, NUM_ENCODER>
{
            ...
            if k.action.is_morse() {
                ...
                match k.state {
                    KeyState::ProcessedButReleaseNotReportedYet(_) => {
                        warn!(
                            "NEED CLEAN: Processing buffering TAP keys with post tap: {:?}",
                            k.event
                        );
                        false
                    }
    ...
    async fn process_key_action(
        ...
        let (keyboard_state_updated, updated_decision_for_cur_key) = self
            .fire_held_keys(decision_for_current_key, decisions)
            .await;
        ...
        match updated_decision_for_cur_key {
            ...
            KeyBehaviorDecision::Ignore => {
                debug!(
                    "Current key is ignored or not buffered, process normally: {:?}",
                    event
                );
            ...
            KeyBehaviorDecision::FlowTap => {
                let action =
                    Self::action_from_pattern(self.keymap.borrow().behavior, key_action, TAP); //tap action
            ...
            match decision {
                        ...
                        && held_key.action.is_morse()
                    {
                        ...
                        match held_key.state {
                            ...
                            KeyState::Released(pattern) => {
                                ...
                                debug!(
                                    "Pattern after released, unilateral tap or flow tap: {:?}",
                                    pattern
                                );
                                let action = Self::action_from_pattern(
                                    self.keymap.borrow().behavior,
                ...
                HeldKeyDecision::PermissiveHold | HeldKeyDecision::HoldOnOtherKeyPress => {
                    if let Some(mut held_key) = self.held_buffer.remove_if(|k| k.event.pos == pos) {
                        let action = self
                            .keymap
                            .borrow_mut()
                            .get_action_with_layer_cache(held_key.event);
                        ...
                        if action.is_morse() {
                            ...
                            match held_key.state {
                                ...
                                KeyState::Released(pattern) => {
                                    debug!(
                                        "pattern after released, permissive hold: {:?}",
                                        pattern
                                    );
                                    let action = Self::action_from_pattern(
                                        self.keymap.borrow().behavior,
                ...
                HeldKeyDecision::Release => {
                    ...
                    if let Some(mut held_key) = self.held_buffer.remove_if(|k| k.event.pos == pos) {
                        ...
                        debug!(
                            "Processing current key before releasing: {:?}",
                            held_key.event
                        );
                            ...
                            match held_key.state {
                                KeyState::Pressed(_) | KeyState::Holding(_) => {
                                    ...
                                    if let Some(action) = final_action {
                                        ...
                                        self.process_key_action_normal(action, held_key.event)
                                            .await;
                                        held_key.state =
                                            KeyState::ProcessedButReleaseNotReportedYet(action);
                                    }
                ...
                HeldKeyDecision::Normal => {
                    ...
                    let trigger_normal =
                        matches!(decision_for_current_key, KeyBehaviorDecision::CleanBuffer);

                    if trigger_normal
                        && let Some(held_key) = self.held_buffer.remove_if(|k| k.event.pos == pos)
                    {
                    ...
                    if event.pressed {
                        ...
                        match mode {
                            ...
                            MorseMode::HoldOnOtherPress => {
                                ...
                                let _ = decisions.push((
                                    held_key.event.pos,
                                    HeldKeyDecision::HoldOnOtherKeyPress,
                                ));
                                decision_for_current_key = KeyBehaviorDecision::CleanBuffer;
                            }
                            _ => {}
                            ...
                            && mode == MorseMode::PermissiveHold
                        {
                            ...
                            let _ = decisions
                                .push((held_key.event.pos, HeldKeyDecision::PermissiveHold));
                            decision_for_current_key = KeyBehaviorDecision::CleanBuffer;
                        }
                    }
                }
    ...
    async fn process_key_action_inner(
        ...
        send_controller_event(
            ...
            ControllerEvent::Key(event, key_action),
        );
    ...
    async fn process_key_action_normal(&mut self, action: Action, event: KeyboardEvent) {
        match action {
            ...
            Action::Modifier(modifiers) => {
                ...
                self.send_keyboard_report_with_resolved_modifiers(event.pressed)
                    .await;
    ...
    async fn process_action_key(&mut self, key: KeyCode, event: KeyboardEvent) {
        ...
        let key = match key {
            ...
            KeyCode::Again => {
                debug!(
                    "Repeat last key code: {:?} , {:?}",
                    self.last_key_code, event
                );
                self.last_key_code
            }
            _ => key,
    ...
    async fn process_action_mouse(&mut self, key: KeyCode, event: KeyboardEvent) {
        if key.is_mouse_key() {
            ...
            if self.mouse_report.x != 0 && self.mouse_report.y != 0 {
                let (x, y) =
                    self.apply_diagonal_compensation(self.mouse_report.x, self.mouse_report.y);
            ...
            if self.mouse_report.wheel != 0 && self.mouse_report.pan != 0 {
                let (wheel, pan) = self
                    .apply_diagonal_compensation(self.mouse_report.wheel, self.mouse_report.pan);
            ...
            if !matches!(
                key,
                KeyCode::MouseAccel0 | KeyCode::MouseAccel1 | KeyCode::MouseAccel2
            ) {
    ...
    async fn process_user(&mut self, key: KeyCode, event: KeyboardEvent) {
        ...
        if self.process_ergohaven_common_user_key(id, event).await {
            return;
        }
    ...
    async fn process_ergohaven_common_user_key(&mut self, id: u8, event: KeyboardEvent) -> bool {
        match id {
            EH_COMMON_USER_PRINT_INFO if event.pressed => {
                self.eh_send_ascii("Product: Velvet UI\nFW: RMK\n", event)
                    .await;
                true
            }
            EH_COMMON_USER_WORD_NEXT => {
                self.eh_hold_with_mod(KeyCode::Right, ModifierCombination::LCTRL, event)
                    .await;
                true
            }
            EH_COMMON_USER_WORD_PREV => {
                self.eh_hold_with_mod(KeyCode::Left, ModifierCombination::LCTRL, event)
                    .await;
                true
            }
            EH_COMMON_USER_WINDOW_NEXT => {
                self.eh_hold_with_mod(KeyCode::Tab, ModifierCombination::LALT, event)
                    .await;
                true
            }
            EH_COMMON_USER_WINDOW_PREV => {
                self.eh_hold_with_mod(
                    KeyCode::Tab,
                    ModifierCombination::LALT | ModifierCombination::LSHIFT,
                    event,
                )
                .await;
                true
            }
            EH_COMMON_USER_LAYER_NEXT if event.pressed => {
                self.eh_move_layer(true);
                true
            }
            EH_COMMON_USER_LAYER_PREV if event.pressed => {
                self.eh_move_layer(false);
                true
            }
            EH_COMMON_USER_PRINT_INFO | EH_COMMON_USER_LAYER_NEXT | EH_COMMON_USER_LAYER_PREV => {
                true
            }
            _ => false,
        }
    ...
    fn eh_move_layer(&mut self, next: bool) {
        let current = self.keymap.borrow().get_activated_layer();
        let layer = if next {
            if current + 1 >= NUM_LAYER as u8 {
                0
            } else {
                current + 1
            }
    ...
    async fn eh_send_ascii(&mut self, text: &str, event: KeyboardEvent) {
        for byte in text.bytes() {
            let (key, shifted) = from_ascii(byte);
            if shifted {
                self.eh_tap_with_mods(key, ModifierCombination::LSHIFT, event)
                    .await;
            } else {
                self.eh_tap_key(key, event).await;
            }
            Timer::after_millis(2).await;
        }
    }

    async fn eh_hold_with_mod(
        &mut self,
        key: KeyCode,
    ...
    async fn eh_tap_key(&mut self, key: KeyCode, event: KeyboardEvent) {
        self.register_keycode(key, event);
        self.send_keyboard_report_with_resolved_modifiers(true)
            .await;
    ...
    fn register_modifier_key(&mut self, key: KeyCode) {
        ...
        send_controller_event(
            &mut self.controller_pub,
            ControllerEvent::Modifier(self.held_modifiers),
        );
    ...
    fn calculate_mouse_move_unit(&self) -> i8 {
        ...
        let unit = if self.mouse_accel & (1 << 2) != 0 {
            ...
            let linear_term = 2u16
                .saturating_mul(repeat_count)
                .saturating_mul(time_to_max);
    ...
    fn calculate_mouse_wheel_unit(&self) -> i8 {
        ...
        let unit = if self.mouse_accel & (1 << 2) != 0 {
            ...
            let max_unit = (config.wheel_delta as u16)
                .saturating_mul(config.wheel_max_speed_multiplier as u16);
            let unit_range = max_unit - min_unit;
...
mod test {
    ...
    fn create_test_keyboard_with_config(config: BehaviorConfig) -> Keyboard<'static, 5, 14, 2> {
        ...
        static KEY_CONFIG: static_cell::StaticCell<PositionalConfig<5, 14>> =
            static_cell::StaticCell::new();
