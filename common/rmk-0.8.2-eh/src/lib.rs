...
#[cfg(feature = "host")]
use {
    crate::descriptor::ViaReport, crate::hid::HidWriterTrait,
...
>(
    ...
    #[cfg(not(feature = "host"))]
    {
        let storage = Storage::new(flash, storage_config, &behavior_config).await;
        let keymap = RefCell::new(
            KeyMap::new(default_keymap, None, behavior_config, positional_config).await,
        );
        (keymap, storage)
    }
}
...
>(
    ...
    #[cfg(all(not(feature = "_no_usb"), not(feature = "_ble")))]
    {
        let mut usb_builder: embassy_usb::Builder<'_, D> =
            new_usb_builder(usb_driver, rmk_config.device_config);
        ...
        loop {
            match led_reader.read_report().await {
                Ok(led_indicator) => {
                    ...
                    LOCK_LED_STATES.store(
                        led_indicator.into_bits(),
